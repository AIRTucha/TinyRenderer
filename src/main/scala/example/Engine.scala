...
class Engine(val canvas: Canvas) {
  ...
  def createScene(low: Vec3, high: Vec3) =
    Scene(
      canvas.width,
      canvas.height,
      low,
...
case class Scene(
  ...
  def dot(vec: Vec3, color: Color) = {
    val redIndex: Int = (vec.y.asInstanceOf[Int] * width + vec.x
      .asInstanceOf[Int]) * 4
  ...
  def triangle(vec1: Vert, vec2: Vert, vec3: Vert, color: Color) =
    if (dotProduct(Vec3(0, 0, 1), vec1.normal) > 0 ||
      ...
      if (d1 > d2) {
        for (y <- vert1.vertex.y.asInstanceOf[Int] to vert2.vertex.y
               .asInstanceOf[Int])
          scanLine(y, vert1, vert3, vert1, vert2, color)
        for (y <- vert2.vertex.y.asInstanceOf[Int] until vert3.vertex.y
               .asInstanceOf[Int])
